...
admin_chat_id = config['TELEGRAM']['chat_admin_id']
admin_topic_id = int(config['TELEGRAM']['admin_topic_id'])
...
# Обновляем номер версии в таблице Params
        try:
            ...
            WHERE db_version != ?
            """, (current_version,))
            logging.debug(f'Старая запись в базе Params удалена')

            # Вставляем новую запись
            cur.execute("""
            INSERT INTO Params (db_version) 
            VALUES (?)
            """, (current_version,))
            logging.debug(f'Новая запись в базе Params добавлена')
...
def create_message_task(task_data, autor_name, assigned_to, content, task_id, category, type):
    try:
        text = f'<b><u>СОЗДАН</u></b> (Инициатор: {autor_name["firstname"]} {autor_name["realname"]})\n'
        text += f'{type} "<a href="https://task.it25.org/front/ticket.form.php?id={task_id}">{task_data["1"]}</a>" \n'
        text += f'{task_data["80"]}\n'
        text += f'Назначено: {assigned_to} ({category})\n'
        text += f'Описание: {content}'
...
def update_and_close_db(cur, conn, last_timestamp, task_id):
    try:
        # Удаляем старую запись
        cur.execute("""
        DELETE FROM tasks_notifications
        WHERE task_id != ?
        """, (task_id,))
        logging.debug(f'Старая запись в базе удалена')

        # Вставляем новую запись
        cur.execute("""
        INSERT INTO tasks_notifications (task_id, time_task_creation) 
        VALUES (?, ?)
        """, (task_id, last_timestamp))
        logging.debug(f'Новая запись в базе добавлена')
...
def main_func(production):
    ...
    try:
        ...
        # Получение времени запуска демона
        last_hourly_action_time = datetime.now()
        ...
        while True:
            ...
            # Проверка, прошел ли час с последнего выполнения hourly_task
            current_time = datetime.now()
            try:
                if (current_time - last_hourly_action_time).total_seconds() >= 3600:
                    # Выполнить hourly_task
                    tgl.send_message(bot_token, admin_chat_id, admin_topic_id, 'Всё работает!!!')
                    last_hourly_action_time = current_time # Обновить время последнего выполнения
                    logging.info("Сообщение в чат администрирования отправлено!")

            except Exception as e:
                logging.critical(f'Ошибка препятствующая проверке времени работы: {traceback.format_exc()}')
                return False
            ...
            try:
                ...
                logging.critical(f'Ошибка препятствующая формированию списка задач: {traceback.format_exc()}')
                return False
...
if __name__ == '__main__':
    ...
    if main_func(production) == False:
        ...
        tgl.send_message(bot_token, admin_chat_id, admin_chat_id, 'Всё сломалось, чини меня!')
        sys.exit(1)
        
    else:
        logging.info('programm exit success')
        tgl.send_message(bot_token, admin_chat_id, admin_chat_id, 'Меня отключили как надо!')
        sys.exit(0)
